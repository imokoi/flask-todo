    ...
    def authenticate(cls, username, password):
        ...
        if user_info is None:
            ...
            if user_info.password == password:
                ...
                return jsonify(success_result(data=token.decode()))
            else:
                return jsonify(
                    failure_result(code=401, message="Invalid password."))
        ...
        def wrapped_func(*args, **kwargs):
            ...
            if not token:
                ...
                try:
                        ...
                        if user.login_time != payload["data"]["login_time"]:
                            return jsonify(
                                failure_result(
                                    code=401,
                                    message=
                                    "Token has been changed, Please login again."
                                ))
                        else:
                            g.current_user = user
                    ...
                    return jsonify(
                        failure_result(code=401, message="Token Expired!"))
