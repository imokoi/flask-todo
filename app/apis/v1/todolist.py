...
def delete_todo_list():
    ...
    try:
        TodoList.delete_todo_list(list_id=list_id, current_user_id=g.current_user.id)
...
def update_todo_list():
    ...
    try:
        TodoList.update_todo_list(list_id=list_id, new_title=new_title, current_user_id=g.current_user.id)
    except PermissionError:
        return jsonify(failure_result(code=401, message="No Permission"))
    return jsonify(success_result())